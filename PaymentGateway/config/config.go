...
func DefaultCfg() *Configuration {
	...
	return &Configuration{
		...
		NodeConfig: NodeConfig{
			...
			RequestTokenUrl:        "https://torplus-accounting.magicwave.io/api/accounting/request/token",
		},
		DNSResolveURL: "https://cloudflare-dns.com/dns-query?name=%s&type=TXT",
	}
}
...
func ParseConfiguration(configFile string) (*Configuration, error) {
	...
	instance := &Configuration{
		...
		DNSResolveURL: rawConfig.DNSResolveURL,
	}
	...
	if instance.DNSResolveURL == "" {
		instance.DNSResolveURL = defCfg.DNSResolveURL
	}
	return instance, nil
}
...
func ParseConfig() (*Configuration, error) {
	...
	if err != nil {
		...
		if len(os.Args) >= 2 && !(os.Args[1] == "home-root-folder" || os.Args[1] == "--home-root-folder") {
			config.RootApiConfig.Seed = os.Args[1]
			config.Port, err = strconv.Atoi(os.Args[2])
			if err != nil {
				return nil, fmt.Errorf("port supplied, but couldn't be parsed: %v", err)
