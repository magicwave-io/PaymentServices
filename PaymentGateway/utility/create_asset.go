...
//const tokenCreatorSeed = "SAGVI3YEAFYQNJDQNM62DGFKPZPDUZ5WWWNYXB5R6PWYPJ2OWM2IFRNU"
...
func CreateAsset() {
	...
	log.Print(sourceKp.Address())

	issuerKp, err := keypair.ParseFull(issuerSeed)
	if err != nil {
		log.Fatal(err)
	}
	log.Print(issuerKp.Address())
	...
	log.Print(distributionKp.Address())
	...
	if issuerAccountDetail.AccountID != issuerKp.Address() {
		...
		signedTransaction, err := txCreateAccounts.Sign(network.PublicNetworkPassphrase, sourceKp)
		if err != nil {
			log.Fatal(err)
		}
	...
	signedTransaction, err := txCreateTrustLine.Sign(network.PublicNetworkPassphrase, distributionKp)
	...
	signedTransaction, err = txCreateAssets.Sign(network.PublicNetworkPassphrase, issuerKp)
	if err != nil {
		log.Fatal(err)
	}
	_, err = client.SubmitTransaction(signedTransaction)
	...
	signedTransaction, err = txSetOptionsSetHomedomain.Sign(network.PublicNetworkPassphrase, issuerKp)
	if err != nil {
		log.Fatal(err)
	}
	_, err = client.SubmitTransaction(signedTransaction)
