...
func NewClientWithProxy() (*http.Client, error) {
	...
	if err != nil {
		return nil, errors.New("Error creating SOCKS5 proxy")
	}
...
func (r *ResolverController) SetupResolving(w http.ResponseWriter, req *http.Request) {
	...
	log.Infof("SetupResolving start with %s, %s\n", domain, domainKey)
	...
	if !ok {
		...
		log.Info("Starting trySetupResolving")

		resolved, err := r.trySetupResolving(domain, domainKey)
		if err != nil {
		...
		r.resolveCache[domain] = resolved
...
func (r *ResolverController) trySetupResolving(domain string, domainKey string) (string, error) {
	...
	if strings.HasSuffix(domain, ".eth") {
		...
		log.Infof("Starting tor resolve for %s", topLevelDomain)
		// First attempt to resolve exact domain
		dnsNames, err := r.TryResolveByTorProxy(domain)

		if err != nil {

			if netErr, ok := err.(net.Error); ok && netErr.Timeout() {
				log.Infof("Got network timeout for domain resolution, skipping root domain resolution")
			} else {
				log.Infof("Direct resolution failed (%s), attempting root domain(%s): %s", domain, topLevelDomain, err.Error())

				// Use DNS resolving
				dnsNames, err = r.TryResolveByTorProxy(topLevelDomain)
		...
		/*
			if err != nil {
				log.Infof("Retry with stock dns %s", domain)
				dnsNames, err = net.LookupTXT(domain)
			}
		...
		if val, ok := m["torplus"]; ok {
			resolvedName = val
		} else {
			resolvedName = m["magicwave"]
		}
	}
...
func (r *ResolverController) TryResolveByTorProxy(domain string) ([]string, error) {

	log.Infof("TryResolveByTorProxy: %s ", domain)

	if (r.http == nil) || (r.http.Transport == nil) {
		httpTorClient, errTor := NewClientWithProxy()

		if errTor != nil {
			log.Infof("TOR PROXY ERROR: %s", errTor.Error())
			return nil, errTor
		}

		r.http = httpTorClient
	}
	//	msg := new(dns.Msg)
	...
	//		return nil, errors.Errorf("Error packing DNS message")
	//	}

	//	base64Msg := base64.StdEncoding.EncodeToString(rawMsg)

	//torUrl := fmt.Sprintf("https://cloudflare-dns.com/dns-query?name=%s&type=TXT", domain)
	torUrl := fmt.Sprintf("https://dns.google/resolve?name=%s&type=TXT", domain)
	//torUrl := fmt.Sprintf("https://139.162.146.226:30443/dns-query?dns=%s", base64Msg)
	//	_ = base64Msg

	req, err := http.NewRequest(http.MethodGet, torUrl, nil)

	if err != nil {
		return nil, errors.Errorf("Error creating request for dns-query")
	}

	//req.Header.Set("Content-Type", "application/dns-message")
	//req.Header.Set("accept", "application/dns-message")

	log.Infof("TryResolveByTorProxy submit request  %s", domain)

	res, err := r.http.Do(req)
	if err != nil {
		if netErr, ok := err.(net.Error); ok && netErr.Timeout() {
			r.torChannelInTimeout = true
			return nil, err
		} else {
			return nil, errors.Errorf("Error executing request (dns-query): %s - %w", err.Error(), err)
		}
	}

	log.Infof("TryResolveByTorProxy submit end (%d), %s", res.StatusCode, domain)

	defer res.Body.Close()
	data, err := ioutil.ReadAll(res.Body)

	if err != nil {
		return nil, errors.Errorf("Error reading response from stream: %s - %w", err.Error(), err)
	}

	var result EthLinkResult

	if err := json.Unmarshal(data, &result); err != nil {
		return nil, errors.Errorf("Error unmarshalling data (json): %s - %w", err.Error(), err)
	}

	var names = funk.Map(result.Answer, func(u AnswerData) string {
		return strings.Replace(u.Data, "\"", "", -1)
	}).([]string)
...
func (r *ResolverController) DoResolve(w http.ResponseWriter, req *http.Request) {
	...
	if !ok {
		...
		Respond(w, MessageWithStatus(http.StatusNotFound, "Resolution failed/incomplete"))
		return
	...
	return
	//Respond(w, MessageWithStatus(http.StatusNotFound, "Domain resolution not found"))
